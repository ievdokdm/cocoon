...
final class UnifiedCheckRun {
  ...
  static Future<PresubmitGuardConclusion> markConclusion({
    ...
    final logCrumb =
        'markConclusion(${changeCrumb}_$stage, ${state.buildName}, ${state.status}, ${state.attemptNumber})';
    ...
    late final Document presubmitGuardDocument;
    late final Document presubmitCheckDocument;
    // transaction block
    try {
      ...
      presubmitGuardDocument = await firestoreService.getDocument(
        presubmitGuardDocumentName,
        transaction: transaction,
      );
      final presubmitGuard = PresubmitGuard.fromDocument(
        presubmitGuardDocument,
      );
      ...
      presubmitCheckDocument = await firestoreService.getDocument(
        checkDocName,
        transaction: transaction,
      );
      final presubmitCheck = PresubmitCheck.fromDocument(
        presubmitCheckDocument,
      );
      ...
      if (builds?[state.buildName] == null) {
        log.info(
        ...
        return PresubmitGuardConclusion(
          ...
          failed: failed,
          summary:
              'Check run "${state.buildName}" not present in $stage CI stage',
      ...
      if (e.status == 404) {
        ...
        return PresubmitGuardConclusion(
          ...
          details:
              '''
$collectionId document not found for stage "$stage" for $changeCrumb. Got 404 from
Firestore.
